...
class ViT(nn.Module):
    def __init__(self, *, num_outputs, dim, depth, heads, mlp_dim, dim_head = 64,
        ...
        self.pos_emb1D = nn.Parameter(torch.randn(num_clusters, dim)) # adds positional information to tile features

        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim) # custom transformer

        self.to_latent = nn.Identity()
        self.linear_head = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, num_outputs)
        ) # maps tile features, which are reduced to one vector, into gene expression values
        self.device = device
...
def predict(model, dataloader, run=None, verbose=True):
    ...
    wsis = []
    projs = []
    for image, rna_data, wsi_file_name, tcga_project in tqdm(dataloader): # goes through the four things in a batch, except rna_data, which will presumably be used later
        if image == []: continue # skips incomplete items to avoid errors
        image = image.to(model.device)
        wsis.append(wsi_file_name) # record slide info and cancer type
        projs.append(tcga_project)

        pred = model(image) # feed features into model
        preds.append(pred.detach().cpu().numpy()) # collect prediction of RNA expression in numpy form
