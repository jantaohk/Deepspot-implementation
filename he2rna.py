...
class HE2RNA(nn.Module, PyTorchModelHubMixin):
    ...
    """
    # compares how different models fare with each other by computing the top k activations, which are the tiles with the highest predicted expression for a gene
    ...
    def forward(self, x):
        if self.training: # if model is in training mode
            k = int(np.random.choice(self.ks)) # picks a k from list of ks
            return self.forward_fixed_k(x, k) # compute predictions using k highest scoring tiles
        else: # if model is in evaluation mode
            pred = 0
            for k in self.ks: # loop over values of k 
                pred += self.forward_fixed_k(x, int(k)) / len(self.ks) # computes prediction for each k and average their results
            return pred

    def forward_fixed_k(self, x, k): # computes output using top-k tile scores
        mask, _ = torch.max(x, dim=1, keepdim=True) # computes max value across dimension 1 (probably features)
        mask = (mask > 0).float() # change values so that they are either 1 or 0. All numbers that were greater than 0 are 1. This filters out tiles with no data
        x = self.conv(x) * mask # pass x through convolutional layers, * mask removes unwanted tiles
        t, _ = torch.topk(x, k, dim=2, largest=True, sorted=True) # pick the top k highest scoring tiles along dimension 2 (probably tile). t is the tensor of top k activations
        x = torch.sum(t * mask[:, :, :k], dim=2) / torch.sum(mask[:, :, :k], dim=2) # weighted sum of top k tile scores weighted by the mask to ignore invalid tiles. normalise by sum of mask values to get average activation
        return x

    def conv(self, x): # applies 1D convolutions and nonlinearities to input
        x = x[:, x.shape[1] - self.input_dim:] # trim input x to keep last input_dim channels
        for i in range(len(self.layers) - 1): # loop over all convolutional layers except the last one
            x = self.do(self.nonlin(self.layers[i](x))) # apply nonlinear activation then dropout
        x = self.layers[-1](x) # pass through the last layer without activation or dropout
        return x
...
#python "C:\Users\taoyu\OneDrive\Documents\ASTAR\src\he2rna.py" --path_csv "C:\Users\taoyu\OneDrive\Documents\ASTAR\more_tcga\ref_file.csv" --subfolder he2rna --exp_name coad --lr 1e-3 --checkpoint pretrained_models/model.pt --change_num_genes --num_genes 19198 --log 0
